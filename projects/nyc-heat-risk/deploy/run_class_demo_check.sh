...
CLASS_PRESENTATION_PDF="$DOWNLOADS_DIR/NYC_Heating_Risk_Sinifta_Gosterilecek_Sirali_Sunum_Omer_Canbolat.pdf"
CLASS_GUIDE_PDF="$DOWNLOADS_DIR/NYC_Heating_Risk_30_Dakika_Sinif_Anlatim_Rehberi_Omer_Canbolat.pdf"
...
file_check "$CLASS_PRESENTATION_PDF" "class display PDF" 20000
file_check "$CLASS_GUIDE_PDF" "30-minute speaker guide PDF" 20000
...
path = Path("/Users/omer/aws-analytics-pipeline/projects/nyc-heat-risk/reports/class_demo_local_health.json")
data = json.loads(path.read_text())
print(data.get("status", "unknown"))
PY
)"
  add_row "OK" "local dashboard startup proof" "127.0.0.1:8000 /health status=${health_status} during check"
  showcase_tmp="$(mktemp)"
  if curl -fsS --max-time 5 "http://127.0.0.1:8000/showcase" >"$showcase_tmp" 2>/dev/null; then
    if grep -q "NYC Heating Risk | İnteraktif Proje Deneyimi" "$showcase_tmp" && grep -q "Risk keşfi" "$showcase_tmp" && grep -q "NYC risk haritası" "$showcase_tmp"; then
      showcase_size="$(wc -c <"$showcase_tmp" | tr -d ' ')"
      add_row "OK" "modern project site" "/showcase rendered (${showcase_size} bytes)"
    else
      add_row "WARN" "modern project site" "/showcase rendered but expected markers were missing"
    fi
  else
    add_row "WARN" "modern project site" "/showcase did not respond during local check"
  fi
  rm -f "$showcase_tmp"
else
  add_row "WARN" "local dashboard startup proof" "not running on 127.0.0.1:8000; start with \`make -C $PROJECT_ROOT serve\`"
fi
...

{
  ...
  echo "## Demo Order"
  echo
  echo "1. Open the class display PDF: \`$CLASS_PRESENTATION_PDF\`."
  echo "2. Show the QR slide and let classmates open the brochure."
  echo "3. Run \`make -C $PROJECT_ROOT demo-proof\`."
  echo "4. If you want the browser project site, run \`make -C $PROJECT_ROOT serve\` and open \`http://127.0.0.1:8000/showcase\`."
  echo "5. For the operational dashboard, open \`http://127.0.0.1:8000/dashboard?top_n=10\`."
  echo "6. Keep the speaker guide open while presenting: \`$CLASS_GUIDE_PDF\`."
  echo "7. Show \`reports/final_project_audit.md\`: it should say \`READY\`, \`0 fail, 0 warn\`."
  echo
  echo "## Cost Safety"
  echo
