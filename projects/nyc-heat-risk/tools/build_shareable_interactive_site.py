  ...
  <style>
    ...
    *{box-sizing:border-box} html{scroll-behavior:smooth}
    body{margin:0;color:var(--ink);font-family:"Avenir Next","Gill Sans","Trebuchet MS",sans-serif;background:
      radial-gradient(circle at 8% 6%,rgba(216,95,66,.28),transparent 28%),
      radial-gradient(circle at 82% 0%,rgba(18,61,52,.24),transparent 28%),
      linear-gradient(135deg,#f6ead5 0%,#e6f0ea 45%,#fff8ea 100%);overflow-x:hidden}
    ...
    .shell{width:min(1220px,calc(100% - 34px));margin:auto;padding:24px 0 64px;position:relative}
    .topbar{position:sticky;top:10px;z-index:20;display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:18px;padding:10px;border:1px solid var(--line);border-radius:999px;background:rgba(255,248,235,.78);backdrop-filter:blur(18px);box-shadow:0 12px 30px rgba(24,35,29,.08)}
    ...
    .hero{display:grid;grid-template-columns:1.08fr .92fr;gap:22px;min-height:640px;align-items:stretch}
    .panel{border:1px solid var(--line);border-radius:34px;background:rgba(255,248,235,.84);box-shadow:var(--shadow);backdrop-filter:blur(18px)}
    .hero-copy{padding:42px;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden;position:relative}.hero-copy:after{content:"";position:absolute;right:-110px;bottom:-130px;width:360px;height:360px;border-radius:50%;background:radial-gradient(circle,rgba(216,95,66,.28),transparent 69%)}
    ...
    h1,h2,h3,.serif{font-family:Georgia,"Times New Roman",serif} h1{margin:26px 0 20px;font-size:clamp(3.35rem,7.4vw,7.6rem);line-height:.86;letter-spacing:-.08em;max-width:880px}
    ...
    @media(max-width:620px){.shell{width:min(100% - 22px,1220px);padding-top:12px}.hero-copy,.section{padding:20px}.panel{border-radius:24px}h1{font-size:3.1rem}.flow,.cols-3,.cols-4,.hero-metrics,.insight-strip,.hypothesis-grid,.sim-controls,.stat-output,.stat-mini-grid,.tech-card-grid,.stack-ribbon,.tour-route{grid-template-columns:1fr}.phone{min-height:360px}.bar-chart{gap:6px;overflow-x:auto}.bar{min-width:42px}.bar span{font-size:.68rem}.bar label{font-size:.68rem}.choice-card,.card{border-radius:20px}.toast{bottom:12px;border-radius:20px}}
  </style>
</head>
<body>
<main class="shell">
